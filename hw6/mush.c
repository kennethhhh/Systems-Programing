...
int main(int argc, char *argv[]){
   ...
   while((cmd_arr = parseline(argc, argv, &num_pipes)) != NULL){
      ...
      if(strcmp(cmd_arr[0]-> args[0], "cd") == 0){
            ...
            if(num_pipes >= 1){ /* IF THERE ARE PIPES*/
               ...
               else /* LAST PROCESS IN PIPELINE */
               {
                  ...
                  if(cmd_arr[i]->change != 0){ /* MIGHT NEED TO MOVE OUTSIDE PIPELINE CONDTIONAL */
                     if(strcmp(cmd_arr[0]->name, "cd")){
                        chdir(cmd_arr[0]->args[1]);
                     }
               ...
               if(cmd_arr[i]->input != 0 && cmd_arr[0]->change != 0){
                  printf("%d\n", cmd_arr[0]->input);
                  if(dup2(cmd_arr[0]->output, STDIn_FILENO) < 0){
                     perror("dup2");
                     exit(-1);
                  }
               }
               if(cmd_arr[i]->output != 0 && cmd_arr[0]->change != 0){
                  printf("%d\n", cmd_arr[0]->output);
                  if(dup2(cmd_arr[0]->output, STDOUT_FILENO) < 0){
                     perror("dup2");
                     exit(-1);
                  }
               }
