        ...
        (strcmp("ustar  ", header.magic) == 0)){
         ...
         if(flags & V_FLAG){
            ...
            if(header.typeflag == '2')
               ...
               strcat(mode, "-");
            strcat(mode, (fmode & S_IRUSR) ? "r" : "-");
            strcat(mode, (fmode & S_IWUSR) ? "w" : "-");
            strcat(mode, (fmode & S_IXUSR) ? "x" : "-");
            strcat(mode, (fmode & S_IRGRP) ? "r" : "-");
            strcat(mode, (fmode & S_IWGRP) ? "w" : "-");
            strcat(mode, (fmode & S_IXGRP) ? "x" : "-");
            strcat(mode, (fmode & S_IROTH) ? "r" : "-");
            strcat(mode, (fmode & S_IWOTH) ? "w" : "-");
            strcat(mode, (fmode & S_IXOTH) ? "x" : "-");
        ...
        (strcmp("ustar  ", header.magic) == 0)){
         ...
         if(strstr(path, pathname)!=NULL){
          if(flags & V_FLAG){
            ...
            printf("%s %s%8d %s ", mode, ugname, filesize, mtime);
          }
...
void extract_contents(int tfile, int tmp_file, int buf_size, int unpad_bytes){
   ...
   for(i = 0; i < buf_size; i++){
     bytes_read = read(tfile, &tmp, sizeof(char) * 512);
     write(tmp_file, &tmp, sizeof(char) * 512);
   }
   if((bytes_read = read(tfile, &tmp, sizeof(char) * unpad_bytes)) != 0){
     write(tmp_file, &tmp, sizeof(char) * unpad_bytes);
     read(tfile, &tmp, sizeof(char) * (512 - unpad_bytes));
   }
 }
/*
...
*/
void extraction(int tfile, int flags, char *args[], int num_arg){
  char path[SIZE] = {'\0'}, tmp_name[101] = {'\0'}, linkpath[100] = {'\0'};
  struct ustar_header header;
  int fmode, fsize, bytes_read, tmp_file, buf_size = 0, unpad_bytes, i;
  char *named = NULL;
  char TYPE;
    ...
    if((strcmp("ustar", header.magic) == 0) ||
      ...
      for(i = 3; i < num_arg; i++){
        ...
        if(named != NULL){
          ...
          named = strstr(path, args[i]);
        }
      }

      if(named != NULL || num_arg <= 3){
        if(flags & V_FLAG){
          printf("%s\n", path);
        }
        ...
        if(header.typeflag == '5'){
          ...
          strcpy(linkpath, header.linkname);
          symlink(linkpath, path);
        }
      }
      named = NULL;
      clear_string(linkpath, 100);
      clear_string(path, SIZE);
      clear_string(tmp_name, 101);
    }
  }
  close(tfile);
}
